...
class PredictionService:
    """Coordinate inference and response formatting.

    IMPORTANT: Each predictor is responsible for its own preprocessing.
    We always pass the original PIL image so that Grad-CAM overlays have
    access to the original pixel values.  Passing a pre-normalised tensor
    to predict() causes all CAM methods to return None (they check for
    `hasattr(image, "convert")` as the PIL sentinel).
    """
    ...
    async def run_prediction(
        ...
        # ── Inference ─────────────────────────────────────────────────────────
        # Always pass the original PIL image so that each predictor can:
        #   1. Resize / normalise internally for inference
        #   2. Use the original pixels for Grad-CAM / attention overlay
        #
        # Passing a pre-normalised tensor breaks Grad-CAM for every model
        # because they check `hasattr(image, "convert")` as the PIL sentinel
        # and set original_pil=None when given a tensor, returning no heatmap.
        try:
        ...
        if result.gradcam_image is not None:
            try:
                ...
                logger.warning(
                    "Visualization encoding failed: %s",
                    e,
                    extra={"request_id": request_id},
                )
        else:
            logger.warning(
                "Predictor returned no visualization — model=%s",
                model_name,
                extra={"request_id": request_id},
            )
        ...
        logger.info(
            "Prediction done in %.1fms | model=%s class=%s conf=%.3f sev=%.3f "
            "crit=%s disagree=%.3f%s",
            inference_time_ms,
            model_name,
